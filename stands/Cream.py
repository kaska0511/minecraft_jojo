...
class Cream(Common_func):
    ...
    def cancel_stand(self):
        ...
        self.clear_effect()
        self.ext.extention_command(f'effect clear {self.name} minecraft:invisibility')      # 暗闇
    ...
    def damage_within_range(self):
        ...
        self.ext.extention_command(f'execute as @e[distance=..1.3] at @s run damage @e[distance=..1.3,name=!{self.name}] 15 minecraft:indirect_magic by {self.name}')
        # 外側は体力半分を持っていくダメージ量。
        self.ext.extention_command(f'execute as @e[distance=1.3..2.3] at @s run damage @e[distance=1.3..2.3,name=!{self.name}] 10 minecraft:indirect_magic by {self.name}')
