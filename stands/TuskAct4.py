...
class TuskAct4(Common_func):
    ...
    def follow_entity(self):
        if self.summon_flag == False and self.target:
            ...
            self.ext.extention_command(f'execute as {self.name} at @s anchored eyes run summon minecraft:armor_stand ^ ^ ^1 {{attributes:[{{id:"minecraft:scale",base:0.4}}],CustomName:TuskAct4,Invisible:1,Invulnerable:1,Small:1,NoGravity:1}}')
